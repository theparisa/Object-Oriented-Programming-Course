...
public:
    ...
    static inline hh_mm_ss readDuration(std::string duration_str) {
        ...
        std::chrono::seconds total_seconds = std::chrono::hours(hour) + std::chrono::minutes(minute) +
    ...
    template <typename T>
    static inline std::ostringstream& addVectorShortInfoWithLikes(std::ostringstream& oss, const std::vector<T>& vec) {
        for (std::size_t i = 0; i < vec.size(); i++) {
            oss << vec[i]->shortInfoWithLikesToString();
            if (i != vec.size() - 1)
                oss << std::endl;
        }
        return oss;
